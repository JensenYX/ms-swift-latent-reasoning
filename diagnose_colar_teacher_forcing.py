#!/usr/bin/env python
"""Teacher-forcing diagnostics for Qwen3-Omni CoLaR overfit checkpoints.

This script rebuilds the same r=2 compressed reasoning path used by
`colar_plugin/colar_trainer.py`, feeds the gold compressed embeddings through
the text backbone, and reports:

* whether the inference prompt prefix exactly matches the training prefix
* whether `</think>` is present in labels and predicted by lm_head on gold path
* how well latent_policy predicts each next gold compressed embedding
"""
...
def _encode_infer_prompt(template, messages: List[Dict[str, Any]]) -> List[int]:
...
def build_compressed_path(generator, record: Dict[str, Any], r: int) -> Tuple[CompressedPath, Dict[str, Any]]:
    tokenizer = generator.tokenizer
    embed_tokens = generator.embed_tokens
    ...
    input_ids = _as_tensor_1d(encoded["input_ids"], embed_device)
    ...
    try:
        ...
        raise ValueError("sample does not contain exactly the expected thinking markers") from exc
    ...
    with torch.inference_mode():
        base_embeds = embed_tokens(input_ids.unsqueeze(0))[0].to(text_device)
    ...
    if rem:
        ...
        valid = torch.cat([
            torch.ones(n, device=text_device),
            torch.zeros(rem, device=text_device),
        ])
    else:
    ...
    count_norm = count.sqrt() if getattr(cfg, "sqrt_mean", False) else count
...
def diagnose_one(generator, record: Dict[str, Any], idx: int, r: int, top_k: int) -> Dict[str, Any]:
    tokenizer = generator.tokenizer
    cfg = generator.config
    ...
    prefix_ids = path.input_ids[:path.prefix_len].tolist()
    labels = path.labels
    ...
    dist = latent_policy(lp_hidden, temperature=1.0)
    pred = dist.mean[0]
    ...
    next_after_close: Optional[Dict[str, Any]] = None
    if path.answer_ids.numel() > 1 and close_pos < hidden.shape[0]:
        target_id = int(path.answer_ids[1].item())
        logits = lm_head(hidden[close_pos:close_pos + 1].to(generator.lm_head_device))[0].float()
        rank, logprob = _rank_and_logprob(logits, target_id)
        next_after_close = {
            "target_id": target_id,
            "target_text": _token_text(tokenizer, target_id),
            "rank": rank,
            "logprob": logprob,
            "top": _top_tokens(logits, tokenizer, top_k),
        }

    label_probe: Dict[str, Any] = {}
    if labels is not None:
        label_cpu = labels.detach().cpu()
        close_idx = path.close_index_original
        open_idx = debug["open_index_original"]
        reason_start = path.reason_start
        think_labels = label_cpu[open_idx + 1:close_idx]
        label_probe = {
            "label_open": int(label_cpu[open_idx].item()),
            "label_newline": int(label_cpu[open_idx + 1].item()) if open_idx + 1 < close_idx else None,
            "label_first_reason": int(label_cpu[reason_start].item()) if reason_start < close_idx else None,
            "label_close": int(label_cpu[close_idx].item()),
            "label_after_close": int(label_cpu[close_idx + 1].item()) if close_idx + 1 < label_cpu.numel() else None,
            "close_label_is_trained": int(label_cpu[close_idx].item()) == THINK_CLOSE_ID,
            "think_label_fraction": float((think_labels != -100).float().mean().item()) if think_labels.numel() else 0.0,
        }
    ...
    return {
        ...
        "gold_after_close_logits": next_after_close,
    }
...
def print_report(row: Dict[str, Any]) -> None:
    ...
    if labels:
        print(
            f"  labels: close={labels['label_close']} trained={labels['close_label_is_trained']} "
            f"think_label_fraction={labels['think_label_fraction']:.3f}"
        )
    print(
    ...
    print(
        f"  latent: nll_mean={lat['nll']['mean']:.4f} "
    ...
    after = row.get("gold_after_close_logits")
    if after:
        print(
            f"  after_close: target={after['target_id']} {after['target_text']!r} "
            f"rank={after['rank']} logprob={after['logprob']:.4f} "
            f"top1={after['top'][0]['id']} {after['top'][0]['text']!r}"
        )
...
def main() -> None:
    ...
    print(f"[tf] checkpoint: {checkpoint}")
    print(f"[tf] dataset: {dataset}")
    print(f"[tf] model_type={model_type} torch_dtype={torch_dtype} attn_impl={attn_impl} device_map={device_map}")
    ...
    try:
        for idx, record in enumerate(records):
            row = diagnose_one(generator, record, idx=idx, r=args.r, top_k=args.top_k)
            print_report(row)
