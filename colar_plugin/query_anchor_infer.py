"""Query-anchored CoLaR latent inference for Qwen3-Omni.

This is a separate inference path for checkpoints trained with
``query_anchor_plugin.py``.  It leaves the baseline ``colar_infer.py`` untouched.
"""
...
class QueryAnchorInferConfig:
    ...
    residual_source: str = 'hidden'
    hidden_source: str = 'prompt_last'
...
def load_query_anchor_infer_config(
    ...
    return QueryAnchorInferConfig(
        ...
        condition_head=bool(overrides.get('condition_head', meta.get('query_anchor_condition_head', True))),
        condition_interaction=bool(
            overrides.get('condition_interaction', meta.get('query_anchor_condition_interaction', True))),
        condition_input_norm=bool(
            overrides.get('condition_input_norm', meta.get('query_anchor_condition_input_norm', False))),
        residual_anchor=bool(overrides.get('residual_anchor', meta.get('query_anchor_residual', True))),
        norm_preserve=bool(overrides.get('norm_preserve', meta.get('query_anchor_norm_preserve', True))),
        residual_source=str(overrides.get('residual_source', meta.get('query_anchor_residual_source', 'hidden'))),
        hidden_source=str(overrides.get('hidden_source', meta.get('query_anchor_hidden_source', 'prompt_last'))),
        proj_init=str(overrides.get('proj_init', meta.get('query_anchor_proj_init', 'identity'))),
        im_end_id=int(overrides.get('im_end_id', meta.get('query_anchor_im_end_id', 151645))),
...
def load_query_anchor_policy(checkpoint: Path, config: QueryAnchorInferConfig) -> QueryAnchoredLatentPolicy:
    ...
    latent_policy = QueryAnchoredLatentPolicy(
        ...
        condition_head=config.condition_head,
        condition_interaction=config.condition_interaction,
        condition_input_norm=config.condition_input_norm,
        residual_anchor=config.residual_anchor,
        norm_preserve=config.norm_preserve,
        proj_init=config.proj_init,
    )
    try:
...
class QueryAnchoredColarLatentGenerator:
    ...
    def latent_generate(
        ...
        query_pos = self._query_anchor_position(input_ids)
        ...
        assert batch_size == 1, 'Query-anchored CoLaR latent inference currently supports batch_size=1 only.'
        ...
        for _ in range(cfg.max_n_latent_forward):
            ...
            lp_query = query_hidden.to(device=self.lp_device, dtype=torch.float32)
            if cfg.latent_temperature <= 0:
                dist = self.latent_policy(lp_hidden, query_hidden=lp_query, temperature=1.0)
                sampled = dist.mean
            else:
            ...
            current_inputs_embeds = anchored.to(device=self.text_device, dtype=question_embeds.dtype) * cfg.embeds_std
        ...
        for _ in range(cfg.max_new_tokens):
            ...
            if cfg.progress_every > 0 and len(generated_ids) % cfg.progress_every == 0:
                print(f'[infer:query-anchor] answer decode progress: {len(generated_ids)}/{cfg.max_new_tokens} tokens')
    ...
    def _query_anchor_position(self, input_ids: torch.Tensor) -> int:
        if self.config.hidden_source == 'prompt_last':
            return input_ids.shape[1] - 1
        row = input_ids[0].tolist()
        try:
            think_open = row.index(self.config.think_open_id)
            im_end_positions = [i for i, tok in enumerate(row[:think_open]) if tok == self.config.im_end_id]
            return max(0, im_end_positions[-1] - 1)  # final <|im_end|> before assistant/<think>
        except (ValueError, IndexError):
            return input_ids.shape[1] - 1


def build_query_anchor_latent_generator(
    *,
    model_path: str,
    checkpoint: Path,
    ...
    config = load_query_anchor_infer_config(checkpoint, infer_overrides)
    latent_policy = load_query_anchor_policy(checkpoint, config)
    ...
    return QueryAnchoredColarLatentGenerator(model, template, latent_policy, config)
